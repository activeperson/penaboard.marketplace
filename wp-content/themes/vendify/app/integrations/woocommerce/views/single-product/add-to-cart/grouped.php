...
<form class="cart" action="<?php echo esc_url( apply_filters( 'woocommerce_add_to_cart_form_action', $product->get_permalink() ) ); ?>" method="post" enctype='multipart/form-data'>
				...
				foreach ( $grouped_product_columns as $column_id ) {
					...
					switch ( $column_id ) {
						...
						case 'label':
							...
							$value .= '</label>';
							$value .= $grouped_product->get_price_html();
							break;
						case 'stock':
							$value = wc_get_stock_html( $grouped_product );
							break;
						default:
							$value = '';
							break;
	...
	<?php if ( $quantites_required ) : ?>
		...
		<button type="submit" class="single_add_to_cart_button btn btn-primary btn-lg"><?php echo esc_html( $product->single_add_to_cart_text() ); ?></button>
