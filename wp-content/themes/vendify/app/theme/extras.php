...
if ( ! function_exists( 'remove_class_filter' ) ) {
	 ...
	 *
	 * In order to use the core WordPress `remove filter` on a filter added with the callback
	...
	function remove_class_filter( $tag, $class_name = '', $method_name = '', $priority = 10 ) {
		...
		foreach ( (array) $callbacks[ $priority ] as $filter_id => $filter ) {
			...
			if ( get_class( $filter['function'][0] ) === $class_name ) {
				...
				// WordPress 4.7+ use core remove filter since we found the class object.
				if ( isset( $fob ) ) {
 ...
 * @return string
 */
function vendify_generator( $gen, $type ) {
...
add_action( 'get_the_generator_html', 'vendify_generator', 10, 2 );
add_action( 'get_the_generator_xhtml', 'vendify_generator', 10, 2 );
