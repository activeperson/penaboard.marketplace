...
function get_template( $template_name, $args = array(), $template_path = '', $default_path = '' ) {
	...
	// Get template file.
	$located = locate_template( $template_name, $template_path, $default_path );

	// File not exists, display error notice.
	if ( ! file_exists( $located ) ) {
		// Translators: %s Attempted template file.
		_doing_it_wrong( __FUNCTION__, wp_kses_post( sprintf( '%s does not exist.', '<code>' . $located . '</code>' ), $located ), esc_attr( ASTOUNDIFY_WC_PRODUCT_EDITOR_VERSION ) );
		return;
	}
...
function locate_template( $template_name, $template_path = '', $default_path = '' ) {
	...
	if ( ! $template_path ) {
		$template_path = 'wc-product-editor';
	}

	// Set default template path if not set.
	if ( ! $default_path ) {
		$default_path = ASTOUNDIFY_WC_PRODUCT_EDITOR_TEMPLATE_PATH;
	}
	...
	return apply_filters( 'astoundify_wc_product_editor_locate_template', $template, $template_name, $template_path );
}
