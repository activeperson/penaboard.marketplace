...
(function( api, $ ) {
	...
	api.ImageGalleryControl = api.Control.extend({
		...
		ready: function() {
			...
			_.bindAll( control, 'openFrame', 'select', 'resetGallery' );
			...
			control.container.on( 'click keydown', '.reset-button', control.resetGallery );
		},
		...
		initFrame: function() {
			...
			this.frame = wp.media({
				button: {
					text: control.params.labels.frame_button
				},
				states: [
					new wp.media.controller.Library({
						title: control.params.labels.frame_title,
	...
	api.controlConstructor['ImageGallery'] = api.ImageGalleryControl;
