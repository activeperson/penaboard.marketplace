	...
	public function sanitize( $meta_value ) {
		...
		if ( $this->single ) {
			...
			if ( ! is_array( $meta_value ) ) {
				return $has_setting_validation ? new WP_Error( 'expected_array', sprintf( __( 'Expected array value for non-single "%s" meta.', 'astoundify-themecustomizer' ), $meta_key ) ) : null;
			}
			$values = $meta_value;
		}
