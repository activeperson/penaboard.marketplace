...
class Astoundify_ThemeCustomizer_Customize_Register extends Astoundify_ModuleLoader_Module {
	...
	public function customize_register_custom_controls( $wp_customize ) {
		$files = array_merge(
			(array) glob( trailingslashit( astoundify_themecustomizer_get_option( 'install_dir' ) ) . 'control/*.php' ),
			(array) glob( trailingslashit( astoundify_themecustomizer_get_option( 'install_dir' ) ) . 'control/**/*.php' )
		);
	...
	public function customize_register_items( $wp_customize ) {
		foreach ( array( 'panels', 'sections', 'controls' ) as $item ) {
			$files = array_merge(
				(array) glob( trailingslashit( astoundify_themecustomizer_get_option( 'definitions_dir' ) ) . $item . '/*.php' ),
				(array) glob( trailingslashit( astoundify_themecustomizer_get_option( 'definitions_dir' ) ) . $item . '/**/*.php' )
			);
		...
		$wp_customize->register_control_type( 'Astoundify_ThemeCustomizer_Control_ImageGallery' );
	}
