...
function astoundify_themecustomizer_get_mods_like( $pattern ) {
	$defaults = array_merge(
		(array) astoundify_themecustomizer_get_control_group_defaults( 'typography-font-pack' ),
		(array) astoundify_themecustomizer_get_control_group_defaults( 'color-scheme' )
	);
	...
	foreach ( $mods as $key => $value ) {
		if ( preg_match( "/{$pattern}/", $key ) ) {
			$found[ $key ] = apply_filters( "theme_mod_{$key}", $value );
		}
	}
...
function astoundify_themecustomizer_add_css( $data ) {
	Astoundify_ThemeCustomizer_Output_PHPGenerator::add( $data );
}
 ...
 *
 * @since 1.0.0.
 * @param boolean $editor_style Declares if it should build for the editor. Note that some rules can be excluded in the editor.
 * @return string The CSS that is built from the data.
 */
function astoundify_themecustomizer_get_inline_css( $editor_style = false ) {
	...
	switch ( $generator ) {
		case 'scss':
			...
			break;
		case 'php':
			return Astoundify_ThemeCustomizer_Output_PHPGenerator::build( $editor_style );
			break;
	}
}
...
function astoundify_themecustomizer_darken_hex( $hex, $steps ) {
	return Astoundify_ThemeCustomizer_Output_PHPGenerator::darken( $hex, $steps );
}
...
function astoundify_themecustomizer_hex_to_rgb( $hex ) {
	return Astoundify_ThemeCustomizer_Output_PHPGenerator::hex_to_rgb( $hex );
}
 ...
 */
function astoundify_hex_is_light( $color ) {
...
/**
 * Get the current parent theme version.
 *
 * @since 1.3.0
 *
 * @return string
 */
function astoundify_themecustomizer_get_theme_version() {
	return wp_get_theme()->parent() ? wp_get_theme()->parent()->get( 'Version' ) : wp_get_theme()->get( 'Version' );
}
 ...
 * @return string
 */
function astoundify_themecustomizer_sanitize_image_url( $input, $setting ) {
	...
	$image_url = ( $file['ext'] ? $input : $setting->default );

	return esc_url_raw( $image_url );
}

/**
 * Check if the given array is contains int values.
 *
 * @since 1.3.0
 *
 * @return string
 */
function astoundify_themecustomizer_sanitize_array_of_int( $value ) {
	if ( is_array( $value ) ) {
...
/**
 * Check if the given array is contains attribute values.
 *
 * @since 1.3.0
 *
 * @return string
 */
function astoundify_themecustomizer_sanitize_array_of_attr( $value ) {
	if ( is_array( $value ) ) {
