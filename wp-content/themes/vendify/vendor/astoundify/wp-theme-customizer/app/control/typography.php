...
class Astoundify_ThemeCustomizer_Control_Typography extends WP_Customize_Control {
	...
	public function font_family() {
		...
		$this->wp_customize->add_control(
			new Astoundify_ThemeCustomizer_Control_BigChoices(
				$this->wp_customize,
				$key,
				array_merge(
					$this->controls['font-family'], array(
	...
	public function font_size() {
		...
		if ( ! isset( $this->controls['font-size'] ) ) {
			return;
		}
		...
		$this->wp_customize->add_control(
			$key, wp_parse_args(
				$this->controls['font-size'], array(
					...
					'description' => __( 'Only affects large-screen devices', 'astoundify-themecustomizer' ),
					'section'     => $this->section,
				)
			)
		);
	...
	public function font_weight() {
		...
		if ( ! isset( $this->controls['font-weight'] ) ) {
			return;
		}
		...
		$this->wp_customize->add_control(
			$key, wp_parse_args(
				$this->controls['font-weight'], array(
					'type'     => 'select',
	...
	public function line_height() {
		...
		if ( ! isset( $this->controls['line-height'] ) ) {
			return;
		}
		...
		$this->wp_customize->add_control(
			$key, wp_parse_args(
				$this->controls['line-height'], array(
					'type'        => 'number',
					'input_attrs' => array(
						'min'  => 1,
						'max'  => 5,
