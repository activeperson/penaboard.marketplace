 ...
 * @since 1.0.0
 */
class Astoundify_ThemeCustomizer_Output_PHPGenerator {
	...
	public static function add( $data ) {
		...
		if ( isset( $data['exclude_editor'] ) ) {
			$entry['exclude_editor'] = $data['exclude_editor'];
		}
	 ...
	 */
	public static function build( $editor_style = false ) {
		if ( ! self::has_rules() ) {
			return '';
		}
		...
		foreach ( self::$data as $query => $ruleset ) {
			...
			foreach ( $ruleset as $rule ) {
				if ( $editor_style && isset( $rule['exclude_editor'] ) && $rule['exclude_editor'] ) {
					continue;
				}
	...
	public static function darken( $hex, $steps ) {
		...
		if ( is_array( $steps ) ) {
			...
			$steps   = wp_parse_args( $steps, $default );
			$r       = max( 0, min( 255, $r + $steps['r'] ) );
			$g       = max( 0, min( 255, $g + $steps['g'] ) );
			$b       = max( 0, min( 255, $b + $steps['b'] ) );
		} /* Steps is string, use it to darken all RGB color */
		else {
		...
		$hash_output = ( strpos( $hex, '#' ) !== false ) ? true : false;
	 ...
	 *
	 * @param string $hex
	 * @param string $format "string" for comma separated rgb output
	 * @return string|array RGB value in comma separated string or array
	 */
	public static function hex_to_rgb( $hex, $format = 'string' ) {

		/* Clean up hash */
		$hex = str_replace( '#', '', $hex );

		/* If it's only 3 letter, repeat it for 6 letter hex */
		if ( strlen( $hex ) == 3 ) {
			$hex = str_repeat( substr( $hex, 0, 1 ), 2 ) . str_repeat( substr( $hex, 1, 1 ), 2 ) . str_repeat( substr( $hex, 2, 1 ), 2 );
		} /* If it's not 6, invalid */
		elseif ( strlen( $hex ) != 6 ) {
			$rgb = array(
				'r' => 0,
				'g' => 0,
			...
			return ( 'string' == $format ) ? implode( ',', $rgb ) : $rgb;
		}
	...
	/**
	 * Transform a RGB value to HEX.
	 *
	 * @since 1.3.0
	 *
	 * @param array $rgb array containing "r", "g", and "b" keys.
	 * @param string $hash_output true will add # to the hex output.
	 * @return string $hex
	 */
	public static function rgb_to_hex( $rgb, $hash_output = true ) {
		$default = array(
			'r' => 0,
			'g' => 0,
			'b' => 0,
