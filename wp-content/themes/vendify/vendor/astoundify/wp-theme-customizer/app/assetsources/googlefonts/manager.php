...
class Astoundify_ThemeCustomizer_AssetSources_GoogleFonts_Manager extends Astoundify_ThemeCustomizer_AssetSource_Font implements Astoundify_ThemeCustomizer_AssetSource_SourceInterface, Astoundify_ThemeCustomizer_AssetSource_FontInterface {
	...
	public function get_families() {
		...
		foreach ( $fonts as $key => $font ) {
			...
			// to request a 'regular' google font we need to use the `400` value.
			if ( in_array( 'regular', $item['variants'] ) ) {
				$replace_key = array_search( 'regular', $item['variants'] );
				$item['variants'][$replace_key] = '400';
			} elseif ( in_array( 'italic', $item['variants'] ) ) {
				$replace_key = array_search( 'italic', $item['variants'] );
				$item['variants'][$replace_key] = '400';
			}

			$variants   = isset( $item['variants'] ) ? $item['variants'] : array();
			$families[] = urlencode( $font . ':' . join( ',', array_intersect( $variants, $weights ) ) );
		}
		return $families;
	}
	...
	public function choose_variants( $variants ) {
		...
		foreach ( $settings as $setting => $value ) {
			if ( false === strpos( $setting, 'weight' ) ) {
				continue;
			}
			...
			if ( in_array( $weight, $variants ) ) {
				$chosen_variants[] = $weight;
			}
		}
