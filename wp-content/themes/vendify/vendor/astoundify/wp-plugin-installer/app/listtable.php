	...
	public function query_dotorg() {
		...
		if ( false === ( $items = get_transient( $key ) ) ) {
			...
			foreach ( $plugins as $plugin ) {
				$plugin = plugins_api( 'plugin_information', wp_parse_args( array( 'slug' => $plugin ), $args ) );
...
<form id="plugin-filter" class="astoundify-plugininstaller" method="post">
	...
	<?php
	do_action( 'astoundify_plugininstaller_list_start', $this );
	$this->display_rows_or_placeholder();
	do_action( 'astoundify_plugininstaller_list_end', $this );
	?>
